...
<template>
  <div class="register-page">
    <ShaderBackground yongshen="huo" shader-type="grainGradient" :overlay-opacity="0.74" />

    <Card class="register-card animate-in w-full max-w-[var(--container-sm)] relative z-10
      ...
      <CardContent class="px-10 pb-10">
        <form @submit.prevent="handleRegister" class="flex flex-col gap-5">
          <div class="flex flex-col gap-2">
            <Label class="text-xs font-semibold tracking-[1px] uppercase text-[var(--text-muted)]">用户名</Label>
            <div class="relative">
              <span class="absolute left-3.5 top-1/2 -translate-y-1/2 text-xs text-[var(--icon-muted)] z-10 pointer-events-none">✦</span>
              <Input v-model="form.username" class="pl-10 h-11 rounded-[10px] bg-[var(--glass-bg)] border-[var(--line-strong)] text-sm" placeholder="请输入用户名" autocomplete="username" />
            </div>
          </div>

          <div class="flex flex-col gap-2">
            <Label class="text-xs font-semibold tracking-[1px] uppercase text-[var(--text-muted)]">邮箱</Label>
            <div class="relative">
              <span class="absolute left-3.5 top-1/2 -translate-y-1/2 text-xs text-[var(--icon-muted)] z-10 pointer-events-none">✧</span>
              <Input v-model="form.email" type="email" class="pl-10 h-11 rounded-[10px] bg-[var(--glass-bg)] border-[var(--line-strong)] text-sm" placeholder="请输入邮箱" autocomplete="email" />
            </div>
          </div>

          <div class="flex flex-col gap-2">
            <Label class="text-xs font-semibold tracking-[1px] uppercase text-[var(--text-muted)]">密码</Label>
            <div class="relative">
              <span class="absolute left-3.5 top-1/2 -translate-y-1/2 text-xs text-[var(--icon-muted)] z-10 pointer-events-none">◇</span>
              <Input v-model="form.password" :type="showPassword ? 'text' : 'password'" class="pl-10 pr-10 h-11 rounded-[10px] bg-[var(--glass-bg)] border-[var(--line-strong)] text-sm" placeholder="请输入密码" autocomplete="new-password" />
              <button type="button" class="absolute right-3.5 top-1/2 -translate-y-1/2 text-[var(--icon-muted)] hover:text-[var(--accent)] transition-colors" @click="showPassword = !showPassword">{{ showPassword ? '◉' : '○' }}</button>
            </div>
          </div>

          <div class="flex flex-col gap-2">
            <Label class="text-xs font-semibold tracking-[1px] uppercase text-[var(--text-muted)]">确认密码</Label>
            <div class="relative">
              <span class="absolute left-3.5 top-1/2 -translate-y-1/2 text-xs text-[var(--icon-muted)] z-10 pointer-events-none">◇</span>
              <Input v-model="form.confirm" type="password" class="pl-10 h-11 rounded-[10px] bg-[var(--glass-bg)] border-[var(--line-strong)] text-sm" placeholder="请再次输入密码" autocomplete="new-password" />
            </div>
          </div>
