...
// GetAlgorithm returns the current algorithm type.
func (s *ZiWeiService) GetAlgorithm() AlgorithmType {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.algorithm
}
...
func computeLiuNianStars(chart *ZiWeiChart, year int) [12][]string {
	...
	stars := [12][]string{}
	for i := 0; i < 12; i++ {
		stars[i] = append([]string(nil), LIU_YAO_STARS...)
	}
	return stars
}
...
func buildFlyingStarAnalysis(ec *engine.ZiweiChart) *FlyingStarAnalysis {
	...
	// Find where each transformed star is located (主星 + 辅星 + 杂耀)
	...
	for b, stars := range ec.AssistantStars {
		for _, s := range stars {
			if strer, ok := s.(fmt.Stringer); ok {
				starToBranch[strer.String()] = b
			}
		}
	}
	for b, stars := range ec.SecondaryStars {
		for _, s := range stars {
			if strer, ok := s.(fmt.Stringer); ok {
				starToBranch[strer.String()] = b
			}
