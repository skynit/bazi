...
func getSeasonElementAdvice(dayGan string, month int) string {
...
func analyzeDayFlowImpact(userChart *bazipkg.BaziResult, dayPillar model.Pillar) string {
	...
	// 生我者（印）加力
	if dayElem == shengWo(userDayElem) || dayZhiElem == shengWo(userDayElem) {
		return fmt.Sprintf("今日%s气生扶日主，运势平稳，易得贵人相助", dayElem)
	}
	// 克我者（官杀）加力
	if dayElem == keWo(userDayElem) || dayZhiElem == keWo(userDayElem) {
		return fmt.Sprintf("今日%s气克身，压力增大，需谨慎行事", dayElem)
	}
	// 我克者（财）加力
	if dayElem == woKe(userDayElem) || dayZhiElem == woKe(userDayElem) {
		return fmt.Sprintf("今日%s财星旺盛，利于求财交易", dayElem)
	}
	// 我生者（食伤）加力
	if dayElem == woSheng(userDayElem) || dayZhiElem == woSheng(userDayElem) {
		return fmt.Sprintf("今日%s食伤泄秀，才华得以发挥", dayElem)
	}
	// 同我者（比劫）
	if dayElem == userDayElem || dayZhiElem == userDayElem {
		return fmt.Sprintf("今日%s比劫旺盛，运势增强但需防竞争", dayElem)
	}
	return "今日流通平稳，无明显增减"
}

// shengWo returns the element that generates me (印星).
func shengWo(elem string) string {
	return map[string]string{"木": "水", "火": "木", "土": "火", "金": "土", "水": "金"}[elem]
}

// keWo returns the element that controls me (官杀).
func keWo(elem string) string {
	return map[string]string{"木": "金", "火": "水", "土": "木", "金": "火", "水": "土"}[elem]
}

// woKe returns the element I control (财).
func woKe(elem string) string {
	return map[string]string{"木": "土", "火": "金", "土": "水", "金": "木", "水": "火"}[elem]
}

// woSheng returns the element I generate (食伤).
