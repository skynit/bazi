...
func tenGodRelation(userGan, dayGan string, isStrong bool) string {
	ue := data.GanElement[userGan]
	de := data.GanElement[dayGan]
	if ue == "" || de == "" {
		return "中性"
	}
...
func checkBranchHarmony(bazi *bazipkg.BaziResult, todayZhi string) int {
...
func hasSelfPunish(bazi *bazipkg.BaziResult, zhi string) bool {
	count := 0
...
func buildBranchDesc(bazi *bazipkg.BaziResult, todayZhi string) string {
	branches := []string{bazi.YearPillar.Zhi, bazi.MonthPillar.Zhi, bazi.DayPillar.Zhi, bazi.HourPillar.Zhi}
	var parts []string
	for _, b := range branches {
		if isLiuHe(b, todayZhi) {
...
func makeSummary(userGan, userElem, todayGan, todayElem, todayZhi, rel string, bazi *bazipkg.BaziResult) (string, string) {
	branchDesc := buildBranchDesc(bazi, todayZhi)
	isStrong := bazi.BodyStrength.Verdict == "身旺"
	...
	default:
		...
		s := fmt.Sprintf("今日天干%s（%s），与日主%s（%s）为忌神关系。%s。上午宜谨慎，下午好转。",
			todayGan, todayElem, userGan, userElem, branchDesc)
...
func scoreWealth(rel string, todayGan, todayZhiElem string) catResult {
	...
	case "正财", "偏财":
		score += 12
		analysis = fmt.Sprintf("财星（%s）透出，利于求财合作。可能有额外收入或理财收益。", data.GanElement[todayGan])
...
func scoreLove(todayZhi string, bazi *bazipkg.BaziResult) catResult {
	score := 60
	var analysis, advice string
...
func scoreNoble(todayZhi string, bazi *bazipkg.BaziResult) catResult {
	score := 60
	var analysis, advice string
...
func scoreStudy(rel string, todayGan string) catResult {
	...
	default:
		analysis = fmt.Sprintf("今日%s气平稳，适合常规学习。按计划推进即可。", data.GanElement[todayGan])
		advice = "保持学习节奏，厚积薄发。"
	}
...
func scoreTravel(todayZhi string, bazi *bazipkg.BaziResult) catResult {
	score := 60
	var analysis, advice string
...
func scoreLawsuit(rel string, todayZhi string, bazi *bazipkg.BaziResult) catResult {
	score := 60
	var analysis, advice string
