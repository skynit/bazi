...
var ganZhiChars = strings.Join(data.Gans[:], "") + strings.Join(data.Zhis[:], "")
...
func parseSolar(req ParseRequest) (*ParsedBirth, error) {
	...
	if year < 1900 || year > 2100 {
		return nil, fmt.Errorf("year out of range (1900-2100): %d", year)
	}
	if month < 1 || month > 12 {
		return nil, fmt.Errorf("month out of range: %d", month)
	}
	if day < 1 || day > daysInMonth(year, month) {
		return nil, fmt.Errorf("day out of range for %d-%02d: %d", year, month, day)
	}
	if hour < 0 || hour > 23 || minute < 0 || minute > 59 {
		return nil, fmt.Errorf("time out of range: %02d:%02d", hour, minute)
	}
...
// Bazi format does not contain calendar date info → returns a sentinel error
// indicating that reverse-lookup from four pillars to date is not yet supported.
func parseBazi(req ParseRequest) (*ParsedBirth, error) {
	...
	return nil, fmt.Errorf("八字逆推日期功能尚未支持，请直接输入公历或农历日期")
}
