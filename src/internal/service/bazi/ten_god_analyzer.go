...
func (a *TenGodAnalyzer) AnalyzeTenGod(proportions []TenGodRatio, dayElem string, bodyStrength BodyStrengthResult) *TenGodAnalysis {
	// Build lookup map
...
func (a *TenGodAnalyzer) buildHealth(m map[string]TenGodRatio, dayElem string) string {
	...
	for god, elem := range godElemMap {
		ratio, ok := m[god]
		if !ok || ratio.Percent < 15 {
			continue
		}
		if h, ok := data.WuxingHealthMap[elem]; ok {
			health = append(health, h.Organs...)
		}
	}
