...
var shenShaV2Cases = []shenShaV2Case{
	...
	{ID: "SS-040", Desc: "寅月天德在丁—全局无丁→月柱无天德",
		...
		Source: "渊海子平"},
	{ID: "SS-042", Desc: "戌月天德在丙—丙戌月→月柱有天德",
		YearP: "庚戌", MonP: "丙戌", DayP: "甲寅", HouP: "庚午", Gender: "MALE",
		Asserts: []shenShaAssert{{"month", "天德"}, {"month", "月德"}},
		Source: "渊海子平"},
}
...
func TestShenShaPrecisionV2(t *testing.T) {
	...
	sb.WriteString("╔══════════════════════════════════════════════╗\n")
	sb.WriteString("║  神煞精度测试V2（已修正柱位预期）         ║\n")
	sb.WriteString("╚══════════════════════════════════════════════╝\n")
	total := pass + fail
	...
	t.Log("\n" + sb.String())
	os.WriteFile("/tmp/bazi_shensha_v2_report.txt", []byte(sb.String()), 0644)
